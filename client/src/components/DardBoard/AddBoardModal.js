...
export default function AddBoardModal({ handleToggleModal, onAddBoard }) {
    ...
    const handleSubmit = e => {
        e.preventDefault();
        if (!name) {
            toast.error('Vui lòng điền tên bảng');
        } else {
            if (!content) {
                toast.error('Vui lòng điền nội dung bảng');
            } else {
                onAddBoard({ name, content });
                handleClose();
            }
        }
    }
    return (
        <div>
            <Dialog onClose={handleToggleModal} aria-labelledby="customized-dialog-title" open={true}>
                <DialogTitle id="customized-dialog-title" onClose={handleToggleModal} style={{ display: 'flex', justifyContent: 'center' }}>
                    Create Board
                 </DialogTitle>
